...
class ExportService:
    ...
    def export_data(self) -> List[ExportResult]:
        ...
        try:
            ...
            if 'database' not in self.db.config or not self.db.config.get('database', ''):
                ...
                # SUGGESTION: Make this count optional with a flag to skip for faster startup with large databases
                self.total_tables = 0
    ...
    def _export_database_tables(self) -> List[ExportResult]:
        ...
        # SUGGESTION: Add support for different file naming patterns
        ...
        # SUGGESTION: Add file header with timestamp, user, and tool version
        ...
        # SUGGESTION: Add option to split output into multiple files for very large exports
        results = []
        ...
        for i, table in enumerate(tables_to_export, 1):
            ...
            if self.total_tables > 0:
                ...
                if self.tables_processed > 1:
                    # Start with 4x higher estimate and decrease as we make progress
                    progress_factor = max(0.25, self.tables_processed/self.total_tables)  # Start at 0.25 (4x) and approach 1.0
                    estimated_total = (elapsed_time / (self.tables_processed * progress_factor)) * self.total_tables
                    remaining_time = estimated_total - elapsed_time
                    logger.info(f"Progress: {progress_percent:.1f}% ({self.tables_processed}/{self.total_tables} tables). TOTAL tables in queue: {self.total_tables}. Estimated time remaining: {self._format_time(remaining_time)}")
            
            # SUGGESTION: Implement progress callback mechanism for UI integration
            logger.info(f"Exporting table {i}/{table_count}: {table}")
            
            try:
                # Export the table to the consolidated schema and data files
                ...
                # Re-raise the exception if we're not continuing on error
                # SUGGESTION: Add 'continue_on_error' option to export configuration
                raise
